...
| `compositing`   | yes    | unavailable          | yes                          | Vector fill/stroke, gradient fills/strokes, **and** image/text draw are faithful for the **full** `agg.BlendMode` set (Porter-Duff plus separable blend modes): solids render directly through a comp-op pixfmt (straight-alpha adaptor); gradients and the text coverage layer composite the recoloured/glyph layer through the same operator using the shape/glyph AA coverage as cover; image blits composite each covered pixel through `comp_op_adaptor_rgba_plain`. |
...
| `dashed_stroke` | yes    | unavailable          | yes                          | `AddDash`/`RemoveAllDashes`/`DashStart`/`GetDashStart` drive AGG `conv_dash` on both backends. Both dash/stroke in **user space** and apply the active transform to the stroked outline last (`conv_transform`), so the dash period and line width scale with the matrix (byte-exact `dashed_stroke_transform`).                                                                                                                                                          |
...
`xor`, `dst-out`) are now in the strict corpus too (`compositing_xor`,
`compositing_dstout`). The C++ backend was always AGG-faithful there — the
straight-alpha adaptor demultiplies the translucent result back to straight on
write, locked by `TestCPPXorBlendIsAGGFaithfulWithAggReal`. The Go _port_ used to
leave premultiplied data in its straight buffer for such results because
`pixfmt_composite`'s `BlendHline`/`BlendSolidHspan` took a SIMD fast path through
premultiplied-destination kernels (`simd.Comp*HspanRGBA`) that skipped the
demultiply-on-write bridge; `compositing_src`/`srcover`/`clear` stayed byte-exact
only because src-over-opaque yields an opaque result, where premultiplied equals
straight. That fast path was removed (the straight composite pixfmt now always
routes through the scalar `CompositeBlenderPlain`; see PLAN.md §5.5), so the two
new scenes agree cross-backend with no pixel exceeding tolerance 2 (max 1 LSB from
float-demultiply vs the C++ integer demultiply).
...
solid/path envelope (observed ~1.3% edge-AA disagreement).

Both backends measure dash lengths **in user space** and apply the active
transform to the stroked outline last — the pipeline is
`path -> dash -> stroke -> transform`, matching AGG's Agg2D and the port's
`addStrokeToRasterizer` (`conv_transform(conv_stroke(...), m)`). The native
stroke functions take a trailing matrix handle and wrap the stroke output in
`agg::conv_transform` (`add_stroke_to_ras`) when it is non-identity; `Stroke()`
passes the user-space path plus the active matrix rather than pre-transforming
the path. So the dash period and line width scale with the transform. A
null/identity matrix takes the original direct-rasterize path, keeping
no-transform scenes byte-identical. The `dashed_stroke_transform` corpus scene
(dashed stroke under scale + rotate) is **byte-exact cross-backend (0/65536)**,
and `TestCPPDashedStrokeUnderTransformDashesInUserSpaceWithAggReal` locks the
convention. This also fixed plain (non-dashed) stroke **width** scaling under a
transform, which shared the same device-vs-user-space gap.
