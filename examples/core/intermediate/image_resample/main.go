...
func newDemo() *demo {
	...
	// C++ on_init: center the quad on the image rectangle, floored.
	dx := frameWidth/2.0 - srcW/2.0
	dy := frameHeight/2.0 - srcH/2.0
	quad := polygonctrl.NewDefaultPolygonCtrl(4, 5.0)
	quad.SetXn(0, math.Floor(0+dx))
	quad.SetYn(0, math.Floor(0+dy))
	quad.SetXn(1, math.Floor(srcW+dx))
	quad.SetYn(1, math.Floor(0+dy))
	quad.SetXn(2, math.Floor(srcW+dx))
	quad.SetYn(2, math.Floor(srcH+dy))
	quad.SetXn(3, math.Floor(0+dx))
	quad.SetYn(3, math.Floor(srcH+dy))
...
func (d *demo) Render(img *agg.Image) {
	...
	renscan.RenderScanlinesAASolid(ras, sl, rb, ctrlColor(icol.NewRGBA(0, 0.3, 0.5, 0.5)))
	...
	filter := imgacc.NewImageFilterLUTWithFilter(imgacc.BilinearFilter{}, true)
	...
	case 0:
		...
		sg := span.NewSpanImageFilterRGBA2x2WithParams[*imageCloneSource, *span.SpanInterpolatorLinear[*transform.TransAffine]](
	...
	case 1:
		tr := transform.NewTransAffineParlToRect(parl, 0, 0, srcW, srcH)
		interp := span.NewSpanInterpolatorLinear[*transform.TransAffine](tr, 8)
		sg := span.NewSpanImageResampleRGBAAffineWithParams[*imageCloneSource](source, interp, filter)
	...
	case 2:
		...
		if tr.IsValid(transform.AffineEpsilon) {
			...
			sg := span.NewSpanImageFilterRGBA2x2WithParams[*imageCloneSource, *span.SpanInterpolatorLinearSubdiv[*transform.TransPerspective]](
				source, interp, filter,
			)
			renderImageSpans(ras, sl, rbPre, sg)
		}
	...
	case 4:
		interp := span.NewSpanInterpolatorPerspectiveLerpQuadToRect(q8, 0, 0, srcW, srcH, 8)
		if interp.IsValid() {
			subdiv := span.NewSpanSubdivAdaptor(interp)
			sg := span.NewSpanImageResampleRGBAWithParams[*imageCloneSource, *span.SpanSubdivAdaptor[*span.SpanInterpolatorPerspectiveLerp]](
	...
	case 5:
		interp := span.NewSpanInterpolatorPerspectiveExactQuadToRect(q8, 0, 0, srcW, srcH, 8)
		if interp.IsValid() {
			subdiv := span.NewSpanSubdivAdaptor(interp)
			sg := span.NewSpanImageResampleRGBAWithParams[*imageCloneSource, *span.SpanSubdivAdaptor[*span.SpanInterpolatorPerspectiveExact]](
				source, subdiv, filter,
			)
			sg.Blur(d.blur.Value())
			renderImageSpans(ras, sl, rbPre, sg)
...
func loadSourceImage() *agg.Image {
	...
	if err != nil {
		panic("image_resample: failed to load spheres image: " + err.Error())
	}
	w, h := img.Width(), img.Height()
	for p := 0; p < w*h; p++ {
